...
function printBanner(TARGET_RUNNER) {
    ...
    console.log(
        ...
        chalk.bold.italic.gray(' by the Modly community v0.0.4h')
    );
    console.log(chalk.green('C2E generates *.spec.ts test files from *.c2e.yaml files located in the local folder for use with Playwright or Cypress.'));
    console.log(chalk.bold.redBright('🚨 WARNING: C2E currently overwrites *.spec.ts files without warning! 🚨 '));
...
function processFilesInFolder() {
    fs.readdirSync(ROOT).filter(fileName => { return fileName.includes(FILTER) }).map(fileName => {
        processFile(fileName);
        // [AR-94] C2E - Remove orphan spec.ts files
    });
};
...
`;
    ...
    let fileName = `${version}-${capability}-${storyboardName}.${TARGET_RUNNER}.spec.ts`
    let fileContent = `
